...
def comm_f(comm):
    for label, name in comm_dict.items():
        if name == comm:
            df_c = yf.download(f'{label}', start='2000-09-01', end=today, interval='1d', auto_adjust=False)
            if df_c.empty:
                return pd.DataFrame()
...
# Metryki historyczne
def comm_data(df, comm):
    if df.empty:
        return pd.DataFrame(columns=['Start_Date', 'End_Date', 'Close_max', 'Close_min', 'Last_close'])
    ...
    close_max = "{:.2f}".format(max_close_value) if pd.notna(max_close_value) else "NaN"
    close_min = "{:.2f}".format(min_close_value) if pd.notna(min_close_value) else "NaN"
    last_close = "{:.2f}".format(last_close_value) if pd.notna(last_close_value) else "NaN"
...
if checkbox_value_rsi:
    ...
    fig_base.add_trace(go.Bar(x=comm_entry_XDays['Date'], y=comm_entry_XDays['RSI'], name='RSI',
                              marker_color='rgba(98,52,18, 0.5)', yaxis='y2'))
    fig_base.update_layout(yaxis2=dict(title='RSI', overlaying='y', side='right'), legend=dict(x=1.1, y=1))

# Świecowy
if show_candlestick:
    st.sidebar.subheader(f'{comm} -> Candlestick chart', divider='red')
    fig_base.add_trace(go.Candlestick(x=comm_entry_XDays['Date'], open=comm_entry_XDays['Open'],
                                      high=comm_entry_XDays['High'], low=comm_entry_XDays['Low'],
...
if show_atr:
    ...
    atr = AverageTrueRange(high=comm_entry_XDays['High'], low=comm_entry_XDays['Low'],
                           close=comm_entry_XDays['Close'], window=atr_period)
    comm_entry_XDays['ATR'] = atr.average_true_range()
    fig_base.add_trace(go.Bar(x=comm_entry_XDays['Date'], y=comm_entry_XDays['ATR'],
                              name='ATR', marker_color='rgba(0, 192, 163, 0.5)', yaxis='y2'))
